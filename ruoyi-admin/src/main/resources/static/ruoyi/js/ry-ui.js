...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			pass: function(id) {
				table.set();
				$.modal.confirm("确定通过该条" + table.options.modalName + "信息吗？", function() {
					var url = ctx + "code/trust/trustProcess";
					var data = { "parentid": id, "status": "pass", "suggestion": "通过"};
					$.operate.submit(url, "post", "json", data);
				});
			},
			// 审核退回审核意见弹窗
			backModel: function(id) {
				$.modal.open("审核意见", ctx + "code/trust/backModal/"+id,400,300);
			},
			// 审核退回信息
			back: function(id,suggestion) {
				if(suggestion == "" || suggestion == null || suggestion == undefined){ suggestion="退回"; }
				table.set();
				var url = ctx + "code/trust/trustProcess";
				var data = { "parentid": id, "status": "back", "suggestion": suggestion};
            ...
            addFull: function(id) {
            	table.set();
            	var url = $.common.isEmpty(id) ? table.options.createUrl : table.options.createUrl.replace("{fid}", id);
			...
			stop: function(id) {
				table.set();
				$.modal.confirm("确定中止该条委托信息吗？", function() {
					var url = "";
					var data = { "id": id };
					$.operate.submit(url, "post", "json", data);
				});
			...
			// 下载模板
			template: function(id,processCode) {
				table.set();
				var url = table.options.templateUrl.replace("{id}", id).replace("{processCode}", processCode);
...
function loadFiledata (id,operateType) {
	var formdata = new FormData();
	formdata.append("id", id);
	$.ajax({
		url: ctx + "code/info/getFiledata",
		data: formdata,
		type: "post",
		processData: false,
		contentType: false,
		success: function(result) {
			...
			$("#fileinput").fileinput({
				language: 'zh', //设置语言
				uploadUrl: ctx + 'code/info/add/'+id, //上传的地址
				showUpload: operateType, //是否显示上传按钮
				showRemove: operateType, //是否显示移除按钮
...
function allNoAllowOperate(){
	$("input").attr('readonly', true);$("textarea").attr('readonly', true);$(':radio').attr('disabled', true);
